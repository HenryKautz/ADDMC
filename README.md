...
This fork changes CUDD's terminal-merging epsilon and exposes it as an option.
`Counter::getModelCount` (`src/implementation/counter.cpp`) calls
`mgr.SetEpsilon(cuddEpsilon)`, where `cuddEpsilon` comes from the new **`--ep`**
...
```
--ep 1e-12 (CUDD's old default):  s wmc 0
--ep 0     (this fork's default): s wmc 2e-30
```
